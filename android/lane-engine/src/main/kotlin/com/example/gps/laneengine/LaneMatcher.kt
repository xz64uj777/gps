...
class LaneMatcher(
    ...
    fun update(
        ...
        val raw = candidates.associate { lane ->
            ...
            val headingScore = if (observation.bearingDegrees != null) {
                ...
                if (laneBearing == null) 0.5
                    ...
                    observation.bearingDegrees, laneBearing
        ...
        val gpsQuality = (1.0 - observation.horizontalAccuracyMeters / 25.0).coerceIn(0.15, 1.0)
        val confidence =
            (0.65 * top.probability + 0.25 * separation + 0.10 * gpsQuality).coerceIn(0.0, 1.0)

        return LaneEstimate(
            top.laneId, confidence, probabilities, confidence >= exactLaneThreshold
        ).also { previous = it }
    }
